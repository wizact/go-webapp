...
async function fetchManifest() {
    if (!manifestCache) {
        const response = await fetch(`/assets/.vite/manifest.json?v=${new Date().getTime()}`);
...
export async function loadModule(moduleName: string) {
    ...
    try {
        ...
        if (moduleEntry) {
            ...
            const module = await import( /* @vite-ignore */ `./${moduleEntry.file}`);
            if (module.default) {
                module.default();
            }
